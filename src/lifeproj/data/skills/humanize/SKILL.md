---
name: humanize
version: 2.8.2-teka.1
description: |
  Write outgoing prose that does not read as AI-generated. Use whenever drafting
  an email, letter, memo, filing, or any document meant for a human reader, and
  whenever asked to humanize existing text. Detects and fixes the common tells:
  "not X, but Y" constructions, rule-of-three triplets, em dash overuse,
  mechanical boldface, inflated significance, AI vocabulary words, filler
  phrases, and generic upbeat conclusions.
license: MIT
---

# Humanize: drafts that sound like a person

Every teka produces outgoing text: email replies, letters, memos, filings.
This skill exists so none of it reads as AI-generated. The pattern catalog is
based on Wikipedia's "Signs of AI writing" page, maintained by WikiProject AI
Cleanup (see Attribution).

## Two modes

### Drafting (the default in a teka)

When composing a fresh draft, apply the patterns below from the first
sentence. Do not write slop and then clean it. Before presenting the draft,
run the self-audit from Process and Output on your own text and fix what it
catches; deliver only the final draft. An email reply does not need to arrive
with an audit trail attached.

Voice: if this teka holds prior outgoing mail from Vlad (look in
`correspondence/`), read a few of his messages first and match how he actually
writes, down to the sign-off. Formal correspondence stays plain and neutral;
that *is* the correct human voice there (see PERSONALITY AND SOUL).

### Rewriting

When given existing text to humanize:
...
In drafting mode, run the same loop privately on your own draft and deliver only the final text.
...
Adapted for lifeproj tekas from [blader/humanizer](https://github.com/blader/humanizer) v2.8.2 (MIT License), which is based on [Wikipedia:Signs of AI writing](https://en.wikipedia.org/wiki/Wikipedia:Signs_of_AI_writing), maintained by WikiProject AI Cleanup. The patterns documented there come from observations of thousands of instances of AI-generated text on Wikipedia. The teka adaptation adds the drafting mode and voice-matching against `correspondence/`; the pattern catalog is unchanged.
